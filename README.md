...
| Snack's hosted iOS and Android previews | Failing for every Snack, including the blank template on SDK 55 and 56 (iOS: "Tried to register two views with the same name RNSScreen"; Android: "Something went wrong" on SDK 56, and an SDK 56 Expo Go that rejects SDK 55). Not caused by this app. The Web preview works. |
...
- **SDK 56** instead of 57, because Snack does not support 57 yet (see the verification log). A short-lived move to SDK 55, made to fix Snack's iOS and Android previews, was reverted once the blank-template check showed those previews fail on every SDK.
